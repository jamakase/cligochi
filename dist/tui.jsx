...
import { getTraitDefinition } from "./engine/traits.js";
...
function PetSidebar(props) {
    const [pet, setPet] = createSignal(loadPet());
    ...
    function maybeToast(api, p, event) {
        if (!shouldShowToast())
            return;
    ...
    return (<Show when={currentPet()} fallback={<box gap={0}>
      ...
      {(p) => {
            ...
            return (<box gap={0}>
            <For each={artLines()}>
              {(line) => (<text fg={props.api.theme.current.accent}>{line}</text>)}
            </For>
              ...
              <box paddingTop={1} gap={0}>
                ...
                <For each={p().getTraits()}>
                  {(traitId) => {
                    ...
                    return (<text fg={props.api.theme.current.textMuted}>
