...
import { Pet } from "cligochi/dist/engine/pet.js"
import { checkTraits, getTraitDefinition } from "cligochi/dist/engine/traits.js"
import { getReaction, shouldShowToast, resetToastThrottle } from "cligochi/dist/engine/reactions.js"
import { getPet, getPets } from "cligochi/dist/pets/registry.js"
import { petStateToCounters, countersToSerialized } from "cligochi/dist/store/state.js"
import { rollRandomPet } from "cligochi/dist/pets/random.js"
import type { PetState, Mood, TraitId, Rarity, Special, PetSpecies, SerializedCounters } from "cligochi/dist/engine/types.js"

const id = "cligochi"
const STATE_FILE = join(homedir(), ".config", "opencode", "plugins", "cligochi-state.json")
...
function rollSpecial(): Special {
  const roll = () => Math.floor(Math.random() * 8) + 3
  return {
    strength: roll(), perception: roll(), endurance: roll(),
    charisma: roll(), intelligence: roll(), agility: roll(), luck: roll(),
  }
}

function defaultState(petId: PetSpecies, name: string): PetState {
  return {
    petId,
    name,
    ...
    traits: [],
    counters: {
      commits: 0, fileSaves: 0, testRuns: 0, testPasses: 0, testFails: 0,
      errors: 0, sessions: 0, nightSessions: 0, forcePushes: 0,
      languages: [], rapidCommits: 0, totalSessionMinutes: 0,
    },
    lastActive: Date.now(),
    createdAt: Date.now(),
  }
...
function loadState(): PetState | null {
  try {
    ...
    return JSON.parse(readFileSync(STATE_FILE, "utf-8")) as PetState
  } catch {
    return null
  }
}
...
function createRandomPet(): Pet {
  ...
  const state = defaultState(rolled.id, rolled.name)
  saveState(state)
  return new Pet(state.petId, state.name, state.stats, state.traits)
}
  ...
  return (
    <Show when={currentPet()} fallback={
      ...
      {(p: () => Pet) => {
        ...
        const special = createMemo(() => loadState()?.special)
      ...
      onSelect={(option) => {
        ...
        if (option.value === "status") {
          ...
          const lines = [
            ...
            "",
            `S.P.E.C.I.A.L.`,
            `STR ${statBar((loadState()?.special.strength ?? 5) * 10)} ${loadState()?.special.strength ?? "?"}`,
            `PER ${statBar((loadState()?.special.perception ?? 5) * 10)} ${loadState()?.special.perception ?? "?"}`,
            `END ${statBar((loadState()?.special.endurance ?? 5) * 10)} ${loadState()?.special.endurance ?? "?"}`,
            `CHR ${statBar((loadState()?.special.charisma ?? 5) * 10)} ${loadState()?.special.charisma ?? "?"}`,
            `INT ${statBar((loadState()?.special.intelligence ?? 5) * 10)} ${loadState()?.special.intelligence ?? "?"}`,
            `AGI ${statBar((loadState()?.special.agility ?? 5) * 10)} ${loadState()?.special.agility ?? "?"}`,
            `LCK ${statBar((loadState()?.special.luck ?? 5) * 10)} ${loadState()?.special.luck ?? "?"}`,
          ]
          if (traits.length > 0) {
